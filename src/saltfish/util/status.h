...
#ifndef SALTFISH_UTIL_STATUS_H_
#define SALTFISH_UTIL_STATUS_H_
...
#ifdef SALTFISH_HEADERS_USE_SHORT_STATUS_MACROS
#include <glog/logging.h>
#endif

#include "saltfish/gutil/macros.h"
#include "saltfish/gutil/port.h"

#include "saltfish/util/saltfish_export.h"
#include "saltfish/util/slice.h"

/// @brief Return the given status if it is not @c OK.
#define SALTFISH_RETURN_NOT_OK(s) do { \
    const ::saltfish::Status& _s = (s);             \
...
#define SALTFISH_RETURN_NOT_OK_PREPEND(s, msg) do { \
    const ::saltfish::Status& _s = (s);                              \
...
#define SALTFISH_RETURN_NOT_OK_RET(to_call, to_return) do { \
    const ::saltfish::Status& s = (to_call);                \
    if (PREDICT_FALSE(!s.ok())) return (to_return);  \
  } while (0)

/// @brief Return the given status if it is not OK, evaluating `on_error` if so.
#define SALTFISH_RETURN_NOT_OK_EVAL(s, on_error) do { \
    const ::saltfish::Status& _s = (s); \
...
#define SALTFISH_WARN_NOT_OK(to_call, warning_prefix) do { \
    const ::saltfish::Status& _s = (to_call);              \
    if (PREDICT_FALSE(!_s.ok())) { \
      SALTFISH_LOG(WARNING) << (warning_prefix) << ": " << _s.ToString();  \
    } \
  } while (0)

/// @brief Log the given status and return immediately.
#define SALTFISH_LOG_AND_RETURN(level, status) do { \
    const ::saltfish::Status& _s = (status);        \
    SALTFISH_LOG(level) << _s.ToString(); \
    return _s; \
  } while (0)

/// @brief If the given status is not OK, log it and 'msg' at 'level' and return the status.
#define SALTFISH_RETURN_NOT_OK_LOG(s, level, msg) do { \
    const ::saltfish::Status& _s = (s);             \
    if (PREDICT_FALSE(!_s.ok())) { \
      SALTFISH_LOG(level) << "Status: " << _s.ToString() << " " << (msg); \
      return _s;     \
    } \
  } while (0)
...
#define SALTFISH_CHECK_OK_PREPEND(to_call, msg) do { \
    const ::saltfish::Status& _s = (to_call);                   \
    SALTFISH_CHECK(_s.ok()) << (msg) << ": " << _s.ToString();  \
  } while (0)

/// @brief If the status is bad, CHECK immediately, appending the status to the
///   logged message.
#define SALTFISH_CHECK_OK(s) SALTFISH_CHECK_OK_PREPEND(s, "Bad status")

/// @brief If @c to_call returns a bad status, DCHECK immediately with
///   a logged message of @c msg followed by the status.
#define SALTFISH_DCHECK_OK_PREPEND(to_call, msg) do { \
    const ::saltfish::Status& _s = (to_call);                   \
    SALTFISH_DCHECK(_s.ok()) << (msg) << ": " << _s.ToString();  \
  } while (0)

/// @brief If the status is bad, DCHECK immediately, appending the status to the
///   logged 'Bad status' message.
#define SALTFISH_DCHECK_OK(s) SALTFISH_DCHECK_OK_PREPEND(s, "Bad status")
...
///   'ret_code' parameter.
#define SALTFISH_RETURN_MAIN_NOT_OK(to_call, msg, ret_code) do { \
    DCHECK_NE(0, (ret_code)) << "non-OK return code should not be 0"; \
    const ::saltfish::Status& _s = (to_call); \
    if (!_s.ok()) { \
      const ::saltfish::Status& _ss = _s.CloneAndPrepend((msg)); \
...
/// @file status.h
///
/// This header is used in both the Saltfish build as well as in builds of
/// applications that use the Saltfish C++ client. In the latter we need to be
/// careful to "namespace" our macros, to avoid colliding or overriding with
/// similarly named macros belonging to the application.
///
/// SALTFISH_HEADERS_USE_SHORT_STATUS_MACROS handles this behavioral change. When
/// defined, we're building Saltfish and:
...
#ifdef SALTFISH_HEADERS_USE_SHORT_STATUS_MACROS
#define RETURN_NOT_OK         SALTFISH_RETURN_NOT_OK
#define RETURN_NOT_OK_PREPEND SALTFISH_RETURN_NOT_OK_PREPEND
#define RETURN_NOT_OK_RET     SALTFISH_RETURN_NOT_OK_RET
#define RETURN_NOT_OK_EVAL    SALTFISH_RETURN_NOT_OK_EVAL
#define WARN_NOT_OK           SALTFISH_WARN_NOT_OK
#define LOG_AND_RETURN        SALTFISH_LOG_AND_RETURN
#define RETURN_NOT_OK_LOG     SALTFISH_RETURN_NOT_OK_LOG
#define CHECK_OK_PREPEND      SALTFISH_CHECK_OK_PREPEND
#define CHECK_OK              SALTFISH_CHECK_OK
#define DCHECK_OK_PREPEND     SALTFISH_DCHECK_OK_PREPEND
#define DCHECK_OK             SALTFISH_DCHECK_OK
#define RETURN_MAIN_NOT_OK    SALTFISH_RETURN_MAIN_NOT_OK

// These are standard glog macros.
#define SALTFISH_LOG              LOG
#define SALTFISH_CHECK            CHECK
#define SALTFISH_DCHECK           DCHECK
#endif

namespace saltfish {

/// @brief A representation of an operation's outcome.
class SALTFISH_EXPORT Status {
 public:
