...
#include "saltfish/util/malloc.h"
...
size_t Status::memory_footprint_excluding_this() const {
  return state_ ? saltfish_malloc_usable_size(state_) : 0;
}

size_t Status::memory_footprint_including_this() const {
  return saltfish_malloc_usable_size(this) + memory_footprint_excluding_this();
}
}  // namespace saltfish